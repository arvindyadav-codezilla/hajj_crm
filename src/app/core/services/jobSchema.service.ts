...
import { JobType, Jobs } from '@core/constants/ApiConstant';
import { AuthService } from '../services/auth.service';  // Import the AuthService

@Injectable({
  providedIn: 'root',  // Make it available to the entire app
})
export class JobSchemaService {
  ...
  getConfig(type: string,lang:string): Observable<any> {
    ...
    return this.http.get(`${this.apiUrl}${Jobs.get(lang)}/${type}/`, { headers });
  }

  createConfig(type: string, data: any,lang:string): Observable<any> {
    const headers = this.getAuthHeaders(); 
    return this.http.post(`${this.apiUrl}${Jobs.Post(lang)}/${type}/`, data, { headers });
  }

  updateConfig(type: string, id: string, data: any,lang:string): Observable<any> {
    const headers = this.getAuthHeaders(); 
    return this.http.put(`${this.apiUrl}${Jobs.Put(lang)}/${type}/${id}/`, data, { headers });
  }

  deleteConfig(type: string, id: string,lang:string): Observable<any> {
    const headers = this.getAuthHeaders();
    return this.http.delete(`${this.apiUrl}${Jobs.Delete(lang)}/${type}/${id}/`, { headers });
  }
