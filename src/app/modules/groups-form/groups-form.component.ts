...
export class GroupsFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    // Get route data and set entityType
    this.route.data.subscribe((data: any) => {
      this.entityType = data?.type; // dynamic entity type (e.g., jobtype, shift_timing)
      console.log('Entity Type:', this.entityType); // Debugging line to verify the type
      if (this.entityType) {
        this.getApi();  // Call API after entityType is available
      } else {
  ...
  loadEntityData() {
    ...
    this.groupsService.getConfig(this.entityType,lang).subscribe((data) => {
  ...
  getApi() {
    ...
    this.languageService.currentLang$.subscribe((lang: string) => {
    this.groupsService.getConfig(this.entityType,lang).subscribe(
      (data) => {
  ...
  handleAction(event: { action: string, rowData: any }) {
    if (event.action === 'edit') {
      this.openModal(event.rowData,'Edit'); // Pass rowData to openModal for editing
  ...
  onDelete(){
    this.languageService.currentLang$.subscribe((lang: string) => {
      this.groupsService.deleteConfig(this.entityType, this.isEditId!,lang).subscribe(
        (response) => {
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      if (this.isEditMode) {
        this.languageService.currentLang$.subscribe((lang: string) => {
        this.groupsService.updateConfig(this.entityType, this.isEditId!, formData,lang).subscribe(
          (response) => {
        ...
        this.languageService.currentLang$.subscribe((lang: string) => {
        this.groupsService.createConfig(this.entityType, formData,lang).subscribe(
          (response) => {
